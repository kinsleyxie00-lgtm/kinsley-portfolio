# About 正文 TextType 动画设计

## 目标

将克制的 React Bits TextType 打字效果应用于 About 页的两段中文正文。动画需要增强文字被逐步书写的感受，同时保持作品集页面的可读性和现有编辑式布局。

## 范围

- 仅为 About 页现有的两段正文添加动画。
- 保留手写 `about me` 标题、人像、简历下载链接、导航、滚动揭示和其他所有页面。
- 复用项目现有的 GSAP 依赖。
- 本次不包含部署。

## 最终正文

第一段：

> 关注 Market Trends、Consumer Insights 与 Brand Communication，参与从 Campaign Planning、内容传播到线下活动执行的完整流程。

第二段：

> 在跨平台协作与项目复盘中持续积累经验，用更清晰的 Strategy 与 Content，提升品牌触达和用户感知。

正文不以“我”开头，语气保持具体、自然，并通过适量英文术语强化市场营销方向，不使用夸大的品牌口号或结果承诺。

## 动画行为

- 正文进入 About 浮层的可视区域后开始输入。
- 第一段只输入一次，每个字符使用约 20–30ms 的轻微可变速度。
- 第一段完成后停顿约 220ms，再开始输入第二段。
- 不删除、不循环；输入完成后正文保持完整显示。
- 输入期间显示一条细竖线光标；第二段完成后隐藏光标。
- 动画开始前预留完整正文的最终高度，避免简历下载链接和周围版式跳动。

## 手写标题融合

- `about me` 手写图片继续使用现有尺寸、裁切和位置。
- 图片使用 `mix-blend-mode: multiply` 与页面米白背景融合，去除近白色图像底与页面背景之间的矩形边缘。
- 不修改原始图片文件。

## 手写标题透明抠图修订

- 以现有 `about me` 手写图片为编辑目标，只移除浅色矩形背景。
- 完整保留原有字形、笔画粗细、字符间距和构图，不重新设计或改写文字。
- 输出带真实 Alpha 通道的版本化 PNG：`about-me-handwritten-cutout-v2.png`。
- 原图片继续保留作为备份，不覆盖或删除。
- 新图片检查通过后由 About 页面引用，并将混合模式恢复为 `normal`，避免透明笔迹与页面背景发生额外颜色叠加。

## 笔画粗细微调

- 在透明抠图版本基础上将笔画视觉粗细减少约 10%。
- 仅对 Alpha 边缘做约 1px 的内收与柔化，保留黑色中心笔画。
- 字形、间距、位置、画布比例和页面显示尺寸保持不变。
- 继续版本化保存，不覆盖原始截图或早期素材。

## 无障碍与降级

- 辅助技术读取完整的最终正文，不逐字播报中间状态。
- 开启 `prefers-reduced-motion: reduce` 时，直接显示两段完整正文，并关闭光标动画。
- 浏览器不支持 IntersectionObserver 时，在组件挂载后直接开始输入，不让正文持续空白。
- 桌面、平板和触屏设备使用一致的内容顺序。

## 组件边界

- 独立的客户端 `TextType` 组件负责可见性检测、输入状态、计时器、光标动画和卸载清理。
- `About` 将现有两段正文作为有序文本数组传入，简历下载链接保持不变。
- About 专用样式负责保留当前宋体/衬线字体、段落节奏、光标表现和预留高度。

## 验证

- TypeScript 与生产构建必须通过。
- 检查第一段到第二段的顺序、最终光标隐藏和简历链接位置稳定。
- 检查关闭并重新打开 About 后动画能重新开始，且不存在遗留计时器。
- 检查 reduced-motion 模式下正文立即完整显示。
- 确认手写标题、人像、导航、Hero 和其他浮层页面不受影响。
